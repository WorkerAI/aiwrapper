...
/* Edited the file (1.0.1) a bit to import it as an ES6 module */

const root = this || {};
const previous_jsonic = root.jsonic;

/**
 * A JSON parser that can parse "bad" JSON. 
 * It can swallow missing quotes and trailing commas.
 */
const jsonic = root.jsonic = function (src) {
...
function stringify(val, opts, depth) {
  ...
  let type = Object.prototype.toString.call(val).charAt(8);
  ...
  if ('N' === type) {
    ...
    if (depth <= opts.depth) {
      ...
      for (i in val) {
        ...
        let str = stringify(val[i], opts, depth)

        if (null != str && pass) {
          let n = i.match(/^[a-zA-Z0-9_$]+$/) ? i : JSON.stringify(i)
          out.push(n + ':' + str)
          j++
        }
      }
...
jsonic.stringify = function (val, callopts) {
  try {
    ...
    let omit = callopts.omit || callopts.o || [];
